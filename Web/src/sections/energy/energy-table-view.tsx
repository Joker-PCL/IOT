...
export function EnergyUsageView({ post, toggle }: { post: EnergyDataProps[]; toggle: TogglePeriodTimeProps }) {
  ...
  useEffect(() => {
    ...
    const rows = days.map((day) => {
      ...
      post.forEach((meter, idx) => {
        ...
        row[`currentValue_${idx}`] = convertEnergyUnit(found?.currentValue, meter.meter_unit, 'kwh') ?? null;
        row[`difference_${idx}`] = convertEnergyUnit(found?.difference, meter.meter_unit, 'kwh') ?? null;
      });
      return row;
    });
    setTableRows(rows);
    ...
    <Card>
      <Box gap={1.5} display="flex" flexDirection="column" alignItems="center" my={5}>
        <Typography ref={titleRef} variant="h4">
          รายงานการใช้พลังงาน (kWh)
        </Typography>
        <Typography ref={subTitleRef} variant="subtitle2">
          ข้อมูล {post[0].meter_data?.meta.startDate} - {post[0].meter_data?.meta.endDate}
        </Typography>
      </Box>
      <Box
        display="flex"
          ...
          >
            <TableHead>
              <TableRow>
                ...
                {post.map((meter, idx) => (
                  <TableCell key={`en-table-header-a-${idx}`} align="center" colSpan={2}>
              ...
              <TableRow>
                {post.map((meter, idx) => (
                  <Fragment key={`en-table-header-b-${idx}`}>
                    <TableCell align="center">เลขมิเตอร์</TableCell>
                ...
                <TableRow key={`en-table-row-${i}`}>
                  ...
                  {post.map((meter, idx) => (
                    <Fragment key={`en-table-row-${idx}`}>
                      <TableCell align="center">{row[`currentValue_${idx}`] > 0 ? row[`currentValue_${idx}`].toLocaleString() : '-'}</TableCell>
                      <TableCell align="center">{row[`difference_${idx}`] > 0 ? row[`difference_${idx}`].toLocaleString() : '-'}</TableCell>
                    </Fragment>
                  ))}
                </TableRow>
              ))}
