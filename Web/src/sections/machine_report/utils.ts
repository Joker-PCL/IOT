...
type ConvertSecondsUnit = 'seconds' | 'minutes' | 'hours';

export function convertSeconds(seconds: number, unit: ConvertSecondsUnit = 'hours'): number {
  const sec = seconds || 0;
  
  switch(unit.toLowerCase()) {
    case 'minutes':
      return (sec / 60); // แปลงเป็นนาที ทศนิยม 2 ตำแหน่ง
    case 'hours':
      return (sec / 3600); // แปลงเป็นชั่วโมง ทศนิยม 2 ตำแหน่ง
    default:
      return sec; // ค่าเดิมเป็นวินาที
  }
}
