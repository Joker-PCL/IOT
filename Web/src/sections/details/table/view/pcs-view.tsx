              ...
              <Table sx={{ widthidth: 800, minWidth: 1000 }}>
                ...
                <TableBody>
                  {dataFiltered.slice(table.page * table.rowsPerPage, table.page * table.rowsPerPage + table.rowsPerPage).map((row) => (
                    <PcsTableRow key={row.mode_pcs_id} row={row} selected={table.selected.includes(row.mode_pcs_id)} />
                  ))}

                  <TableEmptyRows height={68} emptyRows={emptyRows(table.page, table.rowsPerPage, dataPcs.length)} />
