...
async function handleDailyWeekData(bucket, sensorId, today, startDate, endDate) {
  ...
  const fluxQuery = `
    from(bucket: "${bucket}")
      ...
      |> filter(fn: (r) => r["_measurement"] == "device_frmpayload_data_value_${sensorId}")
      |> filter(fn: (r) => r["_field"] == "value")
      |> aggregateWindow(every: 1d, fn: last, offset: 23h59m59s, createEmpty: false)
  `;
  ...
  const data = await Promise.all(
    days.map(async (day, index) => {
      console.log('day', day.date.format('DD/MM/YYYY HH:mm'), day.isFuture, day.isToday);
...
async function handleWeeklyMonthData(bucket, sensorId, today, startDate, endDate) {
  ...
  const fluxQuery = `
    from(bucket: "${bucket}")
      ...
      |> filter(fn: (r) => r["_measurement"] == "device_frmpayload_data_value_${sensorId}")
      |> filter(fn: (r) => r["_field"] == "value")
      |> aggregateWindow(every: 1w, fn: last, offset: 23h59m59s, createEmpty: false)
  `;
...
async function handleMonthlyYearData(bucket, sensorId, today, startDate, endDate) {
  ...
  const fluxQuery = `
    from(bucket: "${bucket}")
      ...
      |> filter(fn: (r) => r["_measurement"] == "device_frmpayload_data_value_${sensorId}")
      |> filter(fn: (r) => r["_field"] == "value")
      |> aggregateWindow(every: 1mo, fn: last, offset: 23h59m59s, createEmpty: false)
  `;
