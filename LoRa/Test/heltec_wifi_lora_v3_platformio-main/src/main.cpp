...
U8G2_SSD1306_128X64_NONAME_F_SW_I2C
    display(U8G2_R0, /* clock=*/OLED_SCL, /* data=*/OLED_SDA,
            /* reset=*/OLED_RESET); // All Boards without Reset of the Display
...
SX1262 Lora = new Module(/*cs*/ LORA_NSS_PIN, /*irq*/ LORA_DIO1_PIN,
                         /*rst*/ LORA_RESET_PIN, /*gpio*/ LORA_BUSY_PIN);
...
void logo() {
  display.clearBuffer();
  display.drawXBM(0, 5, logo_width, logo_height,
...
void WIFISetUp(void) {
  ...
  WiFi.begin("Your WiFi SSID",
             "Your Password"); // fill in "Your WiFi SSID","Your Password"
  delay(100);
  ...
  if (transmitFlag) {
    ...
    if (state == RADIOLIB_ERR_NONE) {
      ...
      display.drawStr(0, 30,
                      String("R_size: " + (String)(str.length()) +
                             " R_rssi:" + (String)(rssiDetection))
                          .c_str());

      display.drawStr(0, 50,
                      String("sent num: " + (String)(counter - 1)).c_str());
      display.sendBuffer();
      delay(100);
