...
/* OTAA para*/
uint8_t devEui[] = { 0xD9, 0x53, 0xFC, 0x04, 0xF3, 0xD6, 0xE4, 0xD8 };
uint8_t appEui[] = { 0xDC, 0x5B, 0x74, 0x9D, 0x45, 0xF9, 0x21, 0x67 };
uint8_t appKey[] = { 0xA8, 0x82, 0x71, 0x7E, 0xCC, 0x0F, 0xFC, 0x05, 0xC0, 0xF6, 0x31, 0x45, 0xCC, 0xB5, 0xD5, 0x89 };
...
static void prepareTxFrame(uint8_t port) {
    ...
    for (int i = 0; i < (sizeof(registers_4byte_member) / sizeof(registers_4byte_member[0])); i++) {
        ...
        uint32_t encodedValue = readAndEncode4byteRegisters(registers_4byte_member[i].reg, registers_4byte_member[i].scaleFactor);

        // ปริ้นค่าของ data
        Serial.printf("%d=> name: %s, encodedValue: %d\n", reg_number, registers_4byte_member[i].name, encodedValue);
    ...
    for (int i = 0; i < (sizeof(registers_2byte_member) / sizeof(registers_2byte_member[0])); i++) {
        ...
        uint16_t encodedValue = readAndEncode2byteRegisters(registers_2byte_member[i].reg, registers_2byte_member[i].scaleFactor);

        // ปริ้นค่าของ data
        Serial.printf("%d=> name: %s, encodedValue: %d\n", reg_number, registers_2byte_member[i].name, encodedValue);
...
void setup() {
  ...
  display.drawString(37, 5, "POLIPHARM");
  display.setFont(ArialMT_Plain_10);
